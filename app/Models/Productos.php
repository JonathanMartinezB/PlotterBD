...
class Productos extends BasicModel implements JsonSerializable
{
    ...
    {
        $result = $this->insertRow("INSERT INTO plotter.productos VALUES (NULL, ?, ?, ?, ?, ?)", array(
                $this->nombres,
                $this->precio,
    ...
    {
        $result = $this->updateRow("UPDATE plotter.productos SET nombres = ?, precio = ?, porcentaje_ganancia = ?, stock = ?, estado = ? WHERE id = ?", array(
                $this->nombres,
                $this->precio,
                $this->porcentaje_ganancia,
                $this->stock,
    ...
    {
        $result = Productos::search("SELECT id FROM plotter.productos where nombres = '" . $nombres. "'");
        if (count($result) > 0) {
            return true;
        } else {
            return false;
