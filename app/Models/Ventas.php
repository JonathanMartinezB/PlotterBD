...
class Ventas extends AbstractDBConnection implements Model, JsonSerializable
{
    ...
    public function getDetalleVenta(): ?array
    {
        ...
        $this->detalleVenta = DetalleVentas::search('SELECT * FROM plotter.detalle_ventas where venta_id = '.$this->id);
    ...
    {
        $query = "INSERT INTO plotter.ventas VALUES (:id,:numero_serie,:cliente_id,:empleado_id,:fecha_venta,:monto,:estado,:created_at,:updated_at)";
    ...
    {
        $query = "UPDATE plotter.ventas SET 
    ...
    public static function searchForId($id) : ?Ventas
    {
        try {
            if ($id > 0) {
                $Venta = new Ventas();
                $Venta->Connect();
                $getrow = $Venta->getRow("SELECT * FROM plotter.ventas WHERE id =?", array($id));
                $Venta->Disconnect();
    ...
    {
        return Ventas::search("SELECT * FROM plotter.ventas");
    }
    ...
    public static function facturaRegistrada($numeroSerie): bool
    {
        ...
        $result = Categorias::search("SELECT id FROM plotter.ventas where numero_serie = '" . $numeroSerie. "'");
