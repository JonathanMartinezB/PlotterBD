...
class DetalleVentas extends AbstractDBConnection implements Model, JsonSerializable
{
    ...
    function insert()
    {
        $query = "INSERT INTO plotter.detalle_ventas VALUES (:id,:venta_id,:producto_id,:cantidad,:precio_venta,:created_at)";
        if($this->save($query)){
    ...
    {
        $query = "UPDATE plotter.detalle_ventas SET 
    ...
    {
        $result = DetalleVentas::search("SELECT id FROM plotter.detalle_ventas where venta_id = '" . $venta_id. "' and producto_id = '" . $producto_id. "'");
        if (count($result) > 0) {
            return true;
        } else {
            return false;
