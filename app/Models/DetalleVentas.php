...
class DetalleVentas extends BasicModel
{
    ...
    {
        $result = $this->insertRow("INSERT INTO plotter.detalle_ventas VALUES (NULL, ?, ?, ?, ?)", array(
    ...
    {
        $result = $this->updateRow("UPDATE plotter.detalle_ventas SET ventas_id = ?, producto_id = ?, cantidad = ?, precio_venta = ? WHERE id = ?", array(
                $this->ventas_id->getId(),
                $this->producto_id->getId(),
                $this->cantidad,
                $this->precio_venta,
    ...
    public static function searchForId($id) : DetalleVentas
    {
        ...
        if ($id > 0) {
            $DetalleVenta = new DetalleVentas();
            $getrow = $DetalleVenta->getRow("SELECT * FROM plotter.detalle_ventas WHERE id =?", array($id));
    ...
    {
        return DetalleVentas::search("SELECT * FROM plotter.detalle_ventas");
    }
    ...
    {
        $result = DetalleVentas::search("SELECT id FROM plotter.detalle_ventas where producto_id = '" . $producto_id. "'");
        if (count($result) > 0) {
            return true;
        } else {
            return false;
