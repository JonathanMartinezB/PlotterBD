...
final class Departamentos extends AbstractDBConnection implements Model, JsonSerializable
{
    ...
    public function getMunicipiosDepartamento(): ?array
    {
        if(!empty($this-> MunicipiosDepartamento)){
            $this-> MunicipiosDepartamento = Municipios::search("SELECT * FROM plotter.municipios WHERE departamento_id = ".$this->id);
    ...
    static function searchForId(int $id): ?Departamentos
    {
        try {
            if ($id > 0) {
                ...
                $getrow = $tmpDepartamento->getRow("SELECT * FROM plotter.departamentos WHERE id =?", array($id));
    ...
    {
        return Departamentos::search("SELECT * FROM plotter.departamentos");
    }
